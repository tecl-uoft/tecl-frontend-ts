  ...
  const remove = () => {
    ...
    if (eventId && eventHash) {
      ScheduleEventService.openRemove(eventId, eventHash)
        .then((res) => {
          alert("Removed your appointment successfully");
            ...
            <div className="flex justify-center px-4 py-3 mx-auto sm:px-6 ">
              <Link to="/scheduling">
                <span className="flex mt-3 rounded-md shadow-sm sm:mt-0 sm:w-auto">
                  <button
                    type="button"
                    className="inline-flex justify-center w-32 px-4 py-2 text-base font-medium leading-6 text-gray-700 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-200 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue sm:text-sm sm:leading-5"
                  >
                    Go Back
                  </button>
                </span>
